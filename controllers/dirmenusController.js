...
exports.GetDirMenus = async (req, res, next) => {
    dbConnection
    try {
                ...
                if (refresh_token == undefined) {
                    const verify_access_token = jwt.verify(access_token, 'access token jwt', async (err, decoded) => {
                        if (err) {
                            ...
                            const [getOrders] = await dbConnection.execute(`call dir_menus(${decoded.Emp_code},${decoded.company_code})`)
                            console.log(getOrders[0].length)
